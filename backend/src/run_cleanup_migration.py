...
sql_file = r"c:\projectms\Hackathon kaggle\supabase\migrations\20260818000000_remove_unnecessary_columns.sql"
    ...
    for port in [5432, 6543]:
        for pw in passwords:
            try:
                ...
                print("Connected! Executing cleanup migration DDL...")
                cursor = conn.cursor()
                cursor.execute(sql)
                conn.commit()
