...
export const ReportPreviewModal = ({ isOpen, onClose, record, prediction, aiReport, user }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-900/60 backdrop-blur-sm flex justify-center p-4 md:p-6 print:p-0 print:bg-white print:static print:h-auto">
      <div className="bg-slate-100 rounded-3xl w-full max-w-5xl flex flex-col shadow-2xl overflow-hidden animate-scale-in border border-slate-200/50 print:shadow-none print:border-none print:bg-white print:rounded-none print:static">
        
        {/* Modal Controls Banner (Hidden in print) */}
        <div className="flex flex-row justify-between items-center bg-white border-b border-slate-200/60 px-6 py-4 print:hidden">
          <div className="flex items-center gap-2">
            <FileText className="text-accent-500" size={20} />
            <h2 className="text-md font-bold text-slate-800">Health Report Preview</h2>
          </div>
          <div className="flex items-center gap-2.5">
            <button 
              onClick={handlePrint}
              className="flex items-center gap-1.5 px-3.5 py-2 text-xs font-semibold text-slate-600 bg-slate-100 hover:bg-slate-200 rounded-xl transition-all"
            >
              <Printer size={14} />
              Print
            </button>
        ...
        <div className="flex-1 overflow-y-auto p-6 md:p-8 bg-slate-200/40 print:p-0 print:bg-white print:overflow-visible">
          
          {/* Simulated Paper Sheet */}
          <div className="bg-white max-w-4xl mx-auto rounded-2xl shadow-xl border border-slate-200/80 p-8 space-y-10 font-sans print:shadow-none print:border-none print:p-0">
            
            {/* Top Diagnostic Header */}
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center border-b pb-6 border-slate-200 gap-4">
              <div className="flex items-center gap-3.5">
                ...
                <div>
                  <h1 className="text-xl font-extrabold text-slate-900 tracking-tight">HealthGuard AI</h1>
                  <p className="text-xs text-teal-600 font-bold uppercase tracking-wider">Automated Diagnostic Assessment</p>
                </div>
              </div>
              <div className="text-left md:text-right text-xs text-slate-400 font-medium">
                <p className="text-slate-700 font-bold">Report ID: <span className="font-mono text-slate-900 select-all">{reportId}</span></p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="bg-slate-50 border border-slate-100 rounded-2xl p-5 flex flex-col justify-between">
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-1">{gender}, {age} Years</p>
                </div>
              ...
              <div className="bg-slate-50 border border-slate-100 rounded-2xl p-5 flex flex-col justify-between">
                ...
                <p className="text-xs text-slate-400 font-semibold mt-4">Calculated by health threat engine</p>
              </div>
              ...
              <div className="bg-slate-50 border border-slate-100 rounded-2xl p-5 flex flex-col items-center justify-center">
                <div className="relative flex items-center justify-center h-28 w-28">
                  ...
                  <div className="absolute flex flex-col items-center">
                    ...
                    <span className="text-[8px] text-slate-400 font-bold uppercase tracking-wider">Health Index</span>
                  </div>
                </div>
              </div>
            ...
            <div className="space-y-6">
              <h3 className="text-sm font-bold uppercase tracking-wider text-slate-800 border-b pb-2 border-slate-100 flex items-center gap-1.5">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                {/* Physical Dimensions */}
                <div>
                  <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2.5">1. Physical Dimensions & Profile</h4>
                  <div className="border border-slate-100 rounded-2xl overflow-hidden text-xs">
                    <div className="flex justify-between p-3 border-b border-slate-50 hover:bg-slate-50">
                      <span className="text-slate-400 font-semibold">Height / Weight</span>
                      <span className="text-slate-800 font-bold">{height} cm / {weight} kg</span>
                    </div>
                    <div className="flex justify-between p-3 border-b border-slate-50 hover:bg-slate-50">
                      <span className="text-slate-400 font-semibold">Calculated BMI</span>
                      <span className="text-slate-800 font-bold">{bmi}</span>
                    </div>
                    <div className="flex justify-between p-3 border-b border-slate-50 hover:bg-slate-50">
                      <span className="text-slate-400 font-semibold">Blood Group</span>
                      <span className="text-slate-800 font-bold">{bloodGroup}</span>
                    </div>
                    <div className="flex justify-between p-3 hover:bg-slate-50">
                      <span className="text-slate-400 font-semibold">Primary Occupation</span>
                      <span className="text-slate-800 font-bold">{occupation}</span>
                    </div>
                  </div>
                </div>
                ...
                {/* Vitals */}
                <div>
                  <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2.5">2. Medical Vitals Data</h4>
                  <div className="border border-slate-100 rounded-2xl overflow-hidden text-xs">
                    <div className="flex justify-between p-3 border-b border-slate-50 hover:bg-slate-50">
                      <span className="text-slate-400 font-semibold">Blood Pressure</span>
                      <span className="text-slate-800 font-bold">{record.blood_pressure || '120/80 mmHg'}</span>
                    </div>
                    <div className="flex justify-between p-3 border-b border-slate-50 hover:bg-slate-50">
                      <span className="text-slate-400 font-semibold">Blood Glucose</span>
                      <span className="text-slate-800 font-bold">{record.blood_sugar ? `${record.blood_sugar} mg/dL` : 'Normal'}</span>
                    </div>
                    <div className="flex justify-between p-3 border-b border-slate-50 hover:bg-slate-50">
                      <span className="text-slate-400 font-semibold">Heart Rate</span>
                      <span className="text-slate-800 font-bold">{record.heart_rate ? `${record.heart_rate} bpm` : 'Normal'}</span>
                    </div>
                    <div className="flex justify-between p-3 hover:bg-slate-50">
                      <span className="text-slate-400 font-semibold">Total Cholesterol</span>
                      <span className="text-slate-800 font-bold">{record.cholesterol ? `${record.cholesterol} mg/dL` : 'Normal'}</span>
                    </div>
                  </div>
                </div>
                ...
                {/* Lifestyle */}
                <div>
                  <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2.5">3. Lifestyle Metrics</h4>
                  <div className="border border-slate-100 rounded-2xl overflow-hidden text-xs">
                    <div className="flex justify-between p-3 border-b border-slate-50 hover:bg-slate-50">
                      <span className="text-slate-400 font-semibold">Tobacco / Smoking Usage</span>
                      <span className="text-slate-800 font-bold">{record.smoking === true || record.smoking === 'Yes' ? 'Yes' : 'No'}</span>
                    </div>
                    <div className="flex justify-between p-3 border-b border-slate-50 hover:bg-slate-50">
                      <span className="text-slate-400 font-semibold">Alcohol Consuming Log</span>
                      <span className="text-slate-800 font-bold">{record.alcohol === true || record.alcohol === 'Yes' ? 'Yes' : 'No'}</span>
                    </div>
                    <div className="flex justify-between p-3 border-b border-slate-50 hover:bg-slate-50">
                      <span className="text-slate-400 font-semibold">Exercise Habit Level</span>
                      <span className="text-slate-800 font-bold">{record.exercise}</span>
                    </div>
                    <div className="flex justify-between p-3 hover:bg-slate-50">
                      <span className="text-slate-400 font-semibold">Sleep Duration & Water</span>
                      <span className="text-slate-800 font-bold">{record.sleep} hrs / {record.water_intake || 2}L</span>
                    </div>
                  </div>
                </div>
                ...
                {/* Medical History */}
                <div>
                  <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2.5">4. Medical History Background</h4>
                  <div className="border border-slate-100 rounded-2xl overflow-hidden text-xs">
                    <div className="flex justify-between p-3 border-b border-slate-50 hover:bg-slate-50">
                      <span className="text-slate-400 font-semibold">Known Chronic Diseases</span>
                      <span className="text-slate-800 font-bold truncate max-w-[120px]">{record.known_diseases || 'None'}</span>
                    </div>
                    <div className="flex justify-between p-3 border-b border-slate-50 hover:bg-slate-50">
                      <span className="text-slate-400 font-semibold">Current Prescription Meds</span>
                      <span className="text-slate-800 font-bold truncate max-w-[120px]">{record.current_medicines || record.medications || 'None'}</span>
                    </div>
                    <div className="flex justify-between p-3 border-b border-slate-50 hover:bg-slate-50">
                      <span className="text-slate-400 font-semibold">Drug / Food Allergies</span>
                      <span className="text-slate-800 font-bold truncate max-w-[120px]">{record.allergies || 'None'}</span>
                    </div>
                    <div className="flex justify-between p-3 hover:bg-slate-50">
                      <span className="text-slate-400 font-semibold">Family Genetic Medical History</span>
                      <span className="text-slate-800 font-bold truncate max-w-[120px]">{record.family_history || 'None'}</span>
                    </div>
                  </div>
                </div>
              ...
              {/* Active Symptoms Box */}
              <div className="bg-rose-50/50 border border-rose-100 rounded-2xl p-4.5 text-xs">
                <h4 className="font-bold text-rose-800 uppercase tracking-wider mb-1.5 flex items-center gap-1.5">
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-4">
                  <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2.5">Organ Risk breakdown</h4>
                  <div className="space-y-3.5">
                    {/* Diabetes */}
                    <div>
                      <div className="flex justify-between text-xs font-semibold text-slate-600 mb-1">
                        <span>Diabetes Risk</span>
                        <span className="font-bold text-slate-800">{diabetesRisk}% ({getLevel(diabetesRisk)})</span>
                      </div>
                    ...
                    <div>
                      <div className="flex justify-between text-xs font-semibold text-slate-600 mb-1">
                        ...
                        <span className="font-bold text-slate-800">{cardioRisk}% ({getLevel(cardioRisk)})</span>
                      </div>
                      <div className="w-full bg-slate-100 h-2 rounded-full overflow-hidden">
                        <div 
                          className={`h-full rounded-full transition-all duration-1000 ${
                    ...
                    <div>
                      <div className="flex justify-between text-xs font-semibold text-slate-600 mb-1">
                        ...
                        <span className="font-bold text-slate-800">{strokeRisk}% ({getLevel(strokeRisk)})</span>
                      </div>
                      <div className="w-full bg-slate-100 h-2 rounded-full overflow-hidden">
                        <div 
                          className={`h-full rounded-full transition-all duration-1000 ${
                    ...
                    <div>
                      <div className="flex justify-between text-xs font-semibold text-slate-600 mb-1">
                        ...
                        <span className="font-bold text-slate-800">{hypertensionRisk}% ({getLevel(hypertensionRisk)})</span>
                      </div>
                      <div className="w-full bg-slate-100 h-2 rounded-full overflow-hidden">
                        <div 
                          className={`h-full rounded-full transition-all duration-1000 ${
                    ...
                    <div>
                      <div className="flex justify-between text-xs font-semibold text-slate-600 mb-1">
                        ...
                        <span className="font-bold text-slate-800">{obesityRisk}% ({getLevel(obesityRisk)})</span>
                      </div>
                      <div className="w-full bg-slate-100 h-2 rounded-full overflow-hidden">
                        <div 
                          className={`h-full rounded-full transition-all duration-1000 ${
                ...
                <div className="bg-slate-50 rounded-2xl p-4 flex flex-col items-center justify-center border border-slate-100">
                  <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2 text-center w-full">Risk Distribution Radar</h4>
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-2">
                <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider">SECTION 6: AI Clinical Diagnosis Explanation</h4>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2">
                ...
                {/* Diet */}
                <div>
                  <h4 className="text-xs font-bold text-slate-800 border-b pb-2 border-slate-100 uppercase tracking-wider flex items-center gap-1.5">
                ...
                <div>
                  ...
                  <div className="border border-slate-100 rounded-2xl overflow-hidden text-xs">
                    {topRisks.map((risk, index) => (
                      <div key={index} className="flex justify-between p-3 border-b border-slate-50 last:border-none hover:bg-slate-50">
                        <span className="font-semibold text-slate-700">{index + 1}. {risk.name}</span>
                ...
                <div>
                  ...
                  <div className="space-y-2.5">
                    {positiveFactors.map((fact, index) => (
                      <div key={index} className="flex gap-2.5 items-start bg-emerald-50/40 border border-emerald-100/50 p-2.5 rounded-xl text-xs">
                        <CheckCircle size={14} className="text-emerald-500 shrink-0 mt-0.5" />
                        <div>
                          <p className="font-bold text-emerald-800">{fact.title}</p>
                          <p className="text-slate-500 font-medium mt-0.5">{fact.desc}</p>
                        </div>
                      </div>
                    ))}
                  </div>
                        ...
                        <tr key={idx} className="hover:bg-slate-50/50">
                          <td className="p-3">
                            <span className={`px-2 py-0.5 rounded text-[9px] font-extrabold ${
                              action.priority === 'HIGH' ? 'bg-red-50 text-red-700' : action.priority === 'MEDIUM' ? 'bg-amber-50 text-amber-700' : 'bg-blue-50 text-blue-700'
                            }`}>
                              {action.priority}
                            </span>
                          </td>
                          <td className="p-3 text-slate-800">{action.recommendation}</td>
                          <td className="p-3 text-slate-400 font-bold">{action.timeline}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
            ...
            <div className="border-t pt-8 text-[10px] text-slate-400 leading-relaxed text-center space-y-2">
              <p className="font-bold text-slate-500 uppercase tracking-wider flex items-center justify-center gap-1.5">
            ...
            {/* Footer */}
            <div className="flex justify-between items-center text-[9px] text-slate-400 border-t pt-4 border-slate-100 font-medium">
