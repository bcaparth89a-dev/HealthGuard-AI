...
def train_stroke_model():
    ...
    data_path = "../../backend/src/data/stroke.csv"
    ...
    # 1. Clean missing values and format columns
    df['bmi'] = pd.to_numeric(df['bmi'].replace('N/A', np.nan), errors='coerce')
    df['bmi'] = df['bmi'].fillna(df['bmi'].median())
    
    # 2. Encode categorical columns to numeric representations
    # gender: Male=1, Female=0, Other=0
    df['gender_numeric'] = df['gender'].map({'Male': 1, 'Female': 0, 'Other': 0}).fillna(0).astype(int)
    
    # smoking_status: never smoked=0, Unknown=0, formerly smoked=1, smokes=2
    smoke_mapping = {'never smoked': 0, 'Unknown': 0, 'formerly smoked': 1, 'smokes': 2}
    df['smoke_numeric'] = df['smoking_status'].map(smoke_mapping).fillna(0).astype(int)
    
    # Selected features that align with health assessment inputs
    feature_cols = ['gender_numeric', 'age', 'hypertension', 'heart_disease', 'avg_glucose_level', 'bmi', 'smoke_numeric']
    ...
    print(f"Successfully saved stroke model to: {model_path}\n")
